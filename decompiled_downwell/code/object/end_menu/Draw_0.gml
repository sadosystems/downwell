if (drawStuff)
{
    ...
    resultStuffy = (ditheryTop + 6 + wholePowan) - 8;
    ...
    nextGoal = tng[gr];
    gemProgy = floor((ditheryTop + 80 + wholePowan2 + rs[5][1]) - 8) + 3;
    ...
    if (rs[5][0])
    {
        ...
        else
        {
            ...
            if (hardUnlockNotif == 2)
            {
                ...
                styleUnlockSpriteFrame += 0.3;
                topTxt = (uny + 20 + powanText) - 4;
                scrGemmyText(__view_get(e__VW.XView, 0) + 80, topTxt, hardUnlockText);
                draw_sprite(sprHardSkull, 1, __view_get(e__VW.XView, 0) + 80, topTxt + 32);
    ...
    if (!goalPassed && sequenceOver && !hardUnlockNotif)
    {
        draw_set_halign(fa_left);
        draw_set_valign(fa_top);
        draw_set_color(c_red);
        ...
        for (i = 0; i <= maxMenu; i += 1)
        {
            ...
            apartByx = 8;
            apartByy = 10;
            menuTexty = ditheryTop + 104 + (i * apartByy) + powany + wholePowan2 + 4;
            menuTextx = vx + 32 + 24 + 16 + (i * apartByx);
